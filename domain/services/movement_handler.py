...
class MovementHandler:
    ...
    def handle_3d_movement(self, direction):
        session = self.session
        camera_controller = session.get_camera_controller()
        camera = session.get_camera()

        if camera_controller is None or camera is None:
            session.message = "3D camera not initialized."
            return False
        ...
        new_x, new_y = camera.grid_position
        session.character.move_to(new_x, new_y)
